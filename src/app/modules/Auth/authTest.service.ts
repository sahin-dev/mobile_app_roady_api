...
import { sendMessage } from "../../../shared/sendMessageTest";
...
const initiateLogin = async (payload: {
  ...
  const messageBody = `Here is your new OTP code to complete the process. ${otp}. It will expire in 5 minutes.`;
  ...
  try {
    ...
    // Uncomment the sendMessage call to actually send the OTP
    await sendMessage(messageBody, payload.phone);
  } catch (err: any) {
...
//   // Send the OTP to user's email
//   if (user.email) await emailSender(user.email, html, "Resend OTP");
...
const resendOtp = async (phone: string) => {
  ...
  await sendMessage(messageBody, phone);

  console.log(messageBody, "OTP sent successfully");
...
const sendOtp = async (
  ...
  if (isSignup) {
    if (user && user.status === UserStatus.ACTIVE)
      throw new ApiError(
        httpStatus.BAD_REQUEST,
        `User with this ${method} already exists.`
      );
  ...
  if (method === "phone") {
    const messageBody = `Here is your OTP code to complete the process: ${otpData.otp}. It will expire in 5 minutes.`;
    await sendMessage(messageBody, identifier);
  }
    ...
    const html = `
      <div style="font-family: Arial, sans-serif;">
        ...
        <p>This OTP will expire in ${
          requestType === RequestType.SIGNUP ? "15" : "5"
        } minutes.</p>
      </div>
    `;
    await emailSender(identifier, html, "Otp verification for Roady App");
